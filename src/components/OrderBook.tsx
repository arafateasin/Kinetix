...
const OrderBook = () => {
  const BASE = 64231.5;
  const [sells, setSells] = useState(() => generateOrders(BASE, 12, false));
  const [buys, setBuys] = useState(() => generateOrders(BASE, 12, true));
  ...
  return (
    <div className="flex flex-col h-full bg-card">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col-reverse">
        {sells.map((o, i) => (
          <div key={`sell-${i}`} className="relative grid grid-cols-3 px-2 py-[2px] text-[11px]">
            <div
              className="absolute inset-0 bg-danger/10"
              style={{ width: `${o.fillPercent}%`, right: 0, left: "auto" }}
            />
            <span className="relative text-danger">{o.price.toLocaleString()}</span>
            <span className="relative text-right text-foreground">{o.amount}</span>
      ...
      <div className="px-2 py-1.5 border-y border-border text-center">
        <span className="text-success font-bold text-sm">64,231.50</span>
        <span className="text-muted-foreground text-[10px] ml-2">≈ $64,231.50</span>
      </div>
      {/* Buys */}
      <div className="flex-1 overflow-y-auto">
        {buys.map((o, i) => (
          <div key={`buy-${i}`} className="relative grid grid-cols-3 px-2 py-[2px] text-[11px]">
            <div
              className="absolute inset-0 bg-success/10"
              style={{ width: `${o.fillPercent}%`, right: 0, left: "auto" }}
            />
            <span className="relative text-success">{o.price.toLocaleString()}</span>
            <span className="relative text-right text-foreground">{o.amount}</span>
            <span className="relative text-right text-muted-foreground">{o.total.toLocaleString()}</span>
          </div>
        ))}
